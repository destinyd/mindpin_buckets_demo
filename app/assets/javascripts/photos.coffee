...
class FolderAdapter extends MindpinBucketsAdapter
  ...
  get_buckets_success: (buckets) ->
    ...
    jQuery.each buckets, ->
      bucket = this
      str += that._str_bucket(bucket.id, bucket.name)
    console.log str
  ...
  create_bucket_success: (bucket) ->
    ...
    console.log bucket
    that = this
    str = @_str_bucket(bucket.id, bucket.name)
  ...
  _str_bucket: (id, name) ->
    "<li class=\"group unbucketed\" data-id=\"#{id}\" id=\"bucket_#{id}\"><a href=\"javascript:;\"><strong>#{name}</strong><!--<span class=\"bucket-meta\">1 photos</span>--><span class=\"bucket-meta\">更新时间</span></a></li>"
